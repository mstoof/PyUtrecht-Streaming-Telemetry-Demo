...
class TelemetryDemo:
    ...
    def demo_capabilities(self):
        ...
        try:
            ...
            print(f"  • gNMI Version: {capabilities.get('gNMI_version', 'Unknown')}")
            print(f"  • Supported Encodings: {', '.join(capabilities.get('supported_encodings', []))}")
                ...
                for notif in result['notification']:
                    for update in notif.get('update', []):
                        ...
                        if isinstance(value, dict):
                            ...
                            print(f"  Errors:     {value.get('in-errors', 0) + value.get('out-errors', 0)}")
    ...
    def demo_subscribe_sample(self):
        ...
        try:
            subscription = [
                {
                    'path': path,
                    'mode': 'sample',
                    'sample_interval': 5 * 1000000000  # 5 seconds in nanoseconds
                }
                for path in paths
            ]
            ...
            for response in self.client.subscribe(subscription=subscription, encoding='json_ietf'):
                update_count += 1
                ...
                if 'update' in response:
                    ...
                    for update in notif.get('update', []):
                        ...
                        if 'name=' in path:
                            iface = path.split('name=')[1].split(']')[0]

                            if 'in-octets' in path:
                                mbps = (value * 8) / 1_000_000
                                print(
                                    f"  {Colors.GREEN}↓{Colors.END} {iface:12} RX: {mbps:8.2f} Mbps  ({value:,} bytes)")
                            elif 'out-octets' in path:
                                mbps = (value * 8) / 1_000_000
                                print(
                                    f"  {Colors.BLUE}↑{Colors.END} {iface:12} TX: {mbps:8.2f} Mbps  ({value:,} bytes)")
    ...
    def demo_subscribe_on_change(self):
        ...
        print(
            f"{Colors.YELLOW}💡 In another terminal, try: docker exec -it clab-pyutrecht-telemetry-demo-spine1 Cli{Colors.END}")
        print(f"{Colors.YELLOW}   Then: conf t → interface ethernet 3 → shutdown{Colors.END}\n")
                    ...
                    for update in notif.get('update', []):
                        ...
                        if 'name=' in path:
                            ...
                            if 'oper-status' in path:
                                color = Colors.GREEN if value == 'UP' else Colors.RED
                                print(f"  Interface {iface}: Operational Status → {color}{value}{Colors.END}")
                            elif 'admin-status' in path:
                                color = Colors.GREEN if value == 'UP' else Colors.YELLOW
